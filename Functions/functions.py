...
def number_devide(a=list, b=int) -> list:
    return [number for number in a if number % b == 0]


listas = [2, 4, 56, 7]

print(number_devide(listas, 2))


def print_numbers(start_number=int, end_number=int):
...
def reverse_digits(number=int) -> str:
    return " ".join([char for char in str(number)[::-1]])
